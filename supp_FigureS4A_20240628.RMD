...
```{r, warning=FALSE, message=FALSE, fig.width = 15, fig.height = 20}
ddsObj.MinoR.all <- readRDS(paste0(main_dir,"Data/ddsObj.MinoR.B1B4H2.2023_0517.RDS"))
...
normalizedCounts <- counts(ddsObj.MinoR.all, normalized = T)
```


```{r, warning=FALSE, message=FALSE, fig.height = 1, fig.width =10}
dd1 <- data.frame(T2 = c(1.5, 4.5, 7.5,
                         10.5, 13.5, 16.5,  19.5,22.5)[1:5],
                  Y=1,
                  T1 = c(#"P", "(5)", "M1(10)",
                    "P", "(5)", "M1(10)", levels(sampleinfo.MinoR.all$Treatment)[1:5])[1:5])

h22 <- ggplot(dd1,aes(x = T2, y = 1, fill= T1 )) + xlim(0,40) +
  geom_bar(stat = "identity") +
  scale_x_continuous(breaks = c(1.5, 4.5, 7.5,
                                10.5, 13.5, 16.5, 
                                19.5,22.5)[1:5], labels =c( "P", "(5)", "M1(10)", levels(sampleinfo.MinoR.all$Treatment)[1:5])[1:5],
                     #c( "DMSO", "5µM\nPBN", "10µM\nPBN", "DMSO","1µM\nPBN", "5µM\nPBN", "10µM\nPBN", "Removed\nPBN"),
...
gene = rownames(normalizedCounts)[ rownames(normalizedCounts) %in% c(paste0("RPL", 1:100), paste0("RPS", 1:100))]
...
ann_colors2 <- list(ann_colors[[1]][1:5])
names(ann_colors2) <- names(ann_colors)
...
pB <- pheatmap::pheatmap(data_subset_norm.2.MinoR[,1:15] , annotation_col = my_sample_col.MinoR[1:15,],  cluster_cols =  F,annotation_colors = ann_colors2 , col = colorRampPalette(rev(c("#67001f", "#d6604d", "#f7f7f7", "#4393c3",
...
grid.arrange(grobs = list( 
                          ...
                          ),nrow =3, heights=c(10,3,30), 
                            ncol =4, widths = c(0.8, 10, 5,2)))
figS4A
```
